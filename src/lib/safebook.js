...
let safebook = {
  ...
  load: (password) => {
    ...
    // TODO: base58 test
    else return safebook.load_from_entropy(password);
  },
  load_from_entropy: (entropy) => {
    let mnemonic = bip39.entropyToMnemonic(
      Buffer.from(safebook.decode(entropy)).toString("hex")
    );
  ...
  load_from_mnemonic: (mnemonic) => {
    ...
    account.entropy = safebook.encode(
  ...
  encrypt: (account, address, message) => {
    // TODO: split into encrypt (low level : Uint8Array) and hide (high level : str)
