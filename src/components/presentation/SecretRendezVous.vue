<template>
  <div>
    ...
    <p>
      Pour plus de détails au sujet des métadonnées vous pouvez lire
      <router-link class="text-blue-500" to="metadata_in_signal"
        >une critique du protocol de signal</router-link
      >.
    </p>
    <H2>Objectif</H2>
    ...
    <p>
      ...
      quelquechose (des messages, etc) à un pair.<br />
      Ainsi, on evite de fuiter des métadonnées sur quels sont nos contacts, etc
    </p>
    <H2
      >Exemple de construction du filtre pour un ensemble de points de
      rendez-vous</H2
    >
    <p>
      Pour simplifier on utilise une seule fonction de hash H<br />
      On rajoute un sel en redefinissant la fonction de hash comme
      <code>H'(x) = H(salt + x)</code>
    </p>
  </div>
</template>
